...
int main()
{
  ...
  //cudaMemPrefetchAsync(gaussianParams, sizeof(GaussianParams), cudaCpuDeviceId, NULL);
 ...
 // cudaMemcpy(host_out_b, gaussianParams->out_b, array_byte_size, cudaMemcpyDeviceToHost);
  ...
  bool success = cv::imwrite("512x512_outputImage_cuda_abc.jpg", outimage);
  ...
  cv::Mat outputImage = cv::imread("512x512_outputImage_cuda_abc.jpg", cv::IMREAD_COLOR);
